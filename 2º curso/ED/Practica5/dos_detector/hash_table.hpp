...
public:
              ...
              keyToInt key_to_int=keyToInt())
    {
        ...
        current_it_=tabla_hash_[0].begin();
        indice_current_=0;
    ...
    {
        //TODO
        /*if(indice_current_<m_ && not tabla_hash_[indice_current_].empty()){
            return true;
        }

        return false;*/
        //return indice_current_<m_ && not tabla_hash_[indice_current_].empty();
        if(indice_current_==tabla_hash_.size()){
            return false;
        }

        for (auto it = tabla_hash_[indice_current_].begin() ; it != tabla_hash_[indice_current_].end(); it++){
            if (it==current_it_){
                return true;
            }
        }
    ...
    {
        //TODO
        /*size_t num_valid_keys=num_of_valid_keys();
        return (float) num_valid_keys/m_;*/

        return valid_keys_/m_;
    ...
    bool has(K const& k) const
    {
        ...
        auto key_hash=hash(key_to_int_(k));
        for (auto it = tabla_hash_[key_hash].begin(); it != tabla_hash_[key_hash].end(); it++){
            if(it->first==k){
                return true;
            }
        }
    ...
    {  //Importante        
        ...
        if(tabla_hash_[indice_current_].empty()){
            return is_found;
        }

        for (current_it_ = tabla_hash_[indice_current_].begin(); current_it_ != tabla_hash_[indice_current_].end(); current_it_++){
            if(current_it_->first==k){
                is_found=true;
                break;
            }
        }
        ...
        if(is_empty()){
        	...
        	auto i = hash(key_to_int_(k));
            
            for(auto it = tabla_hash_[i].begin(); it != tabla_hash_[i].end() and !is_found; it++){
                if(it->first==k){
                    indice_current_=i;
                    current_it_=it;
                    is_found=true;
    ...
    {  //Importante
        ...
        if(find(k)){
            ...
            indice_current_=hash(key_to_int_(k));  //Calcula el índice
            tabla_hash_[indice_current_].push_front(std::make_pair(k, v));
            valid_keys_++;
            current_it_=tabla_hash_[indice_current_].begin();
            if(load_factor()>0.9){  //Si ve que esta demasiado cargado el factor reorganiza toda la tabla
                rehash();
            }
        }
    ...
    {  //Importante
        ...
        auto aux_indice_current=indice_current_;
        auto aux_current_it=current_it_;
        ...
        tabla_hash_[aux_indice_current].erase(aux_current_it);
        valid_keys_--;
    ...
    {  //Importante
        ...
        auto aux_key=current_it_->first;
    ...
    {
        //TODO
        for(size_t i = 0; i < tabla_hash_.size(); i++){
            if(not tabla_hash_[i].empty()){
                current_it_=tabla_hash_[i].begin();
                indice_current_=i;
                break;
            }
        }
    ...
    void goto_next()
    {
        ...
        //TODO
        current_it_++;
        if(current_it_==tabla_hash_[indice_current_].end()){
            size_t i=indice_current_ + 1;
            while(i<tabla_hash_.size() and tabla_hash_[i].empty()){
                i++;
            }

            if(i<tabla_hash_.size() and not tabla_hash_[i].empty()){
                current_it_=tabla_hash_[i].begin();
                indice_current_=i;
                assert(is_valid());
            }
                ...
                indice_current_=tabla_hash_.size();
            }
        }
...
protected:
    ...
    //Para la tabla
    std::vector<std::list<std::pair<K, V>>> tabla_hash_;
    size_t m_;

    //Para la conversión
    keyToInt key_to_int_;

    //Parámetros (constantes) para la función hash
    uint64_t a_;
    uint64_t b_;
    uint64_t p_;

    //Variables para representar el current
