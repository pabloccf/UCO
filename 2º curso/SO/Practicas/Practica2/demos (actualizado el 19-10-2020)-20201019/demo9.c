...
int main(void)
{
   ...
   */
   ...
   {
      printf("Se ha producido un error en pthread_join()\n");
      printf("El valor de la variable error es: %d, y su significado es: %s\n", error, strerror(error));
      // La funcion pthread_join() no mete nada en errno (consultar openGroup), por eso el valor de errno
      // no coincide con el valor de la variable error
      perror("Usando funcion perror que muestra el valor de errno ");
   }
   
   
   printf("Soy el main() con PID %d. Valor de retorno de la hebra: %ld\n",getpid(), *(long *)ret);
   pthread_exit(NULL);
}
