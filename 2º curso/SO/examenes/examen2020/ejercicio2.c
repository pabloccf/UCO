...
int main(int argc, const char* argv[]){
    if(argc<2){
        fprintf(stderr, "Error, numero de argumentos no validos. Debe introducir el numero de hilos que desea crear\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < nhilos; i++){
        indiceglobal[i]= i + 1;
        if(pthread_create(&hilos[i], NULL, (void*) suma, (void*) &indiceglobal[i])){
