...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "abc:xy:")) != -1) {
        ...
        case '?':              // Opcion no reconocida o INCOMPLETA (sin argumento). Probar
                               ...
                               // %$>./a.out -m
            if (optopt == 'c' || optopt == 'y') // Para el caso de que 'c' o 'y' no tenga el argumento
                               // obligatorio.
                fprintf(stderr,
    ...
    */
    ...
    if (yvalue == NULL)
        yvalue = "ValorPorDefecto";

    // Para visualizar que opciones se han activado y sus argumentos
    printf("aflag = %d, bflag = %d, cvalue = %s, xflag = %d, yvalue = %s\n", aflag, bflag, cvalue, xflag, yvalue);
