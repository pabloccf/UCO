...
public class CriticaDAO {
    ...
    public void crearCritica(CriticaDTO critica) {
        try{
            ...
            PreparedStatement ps = connection.prepareStatement("INSERT INTO `Criticas` (Titulo, Puntuacion, Texto, Valoracion, Owner, CantidadValoraciones, TituloEspectaculo) values (?,?,?,?,?,?,?)");
                ...
                ps.setString(7, critica.getTituloEspectaculo());
    ...
    public void eliminarCritica(int id) {
        try {
            ...
            PreparedStatement ps = connection.prepareStatement("DELETE FROM Criticas WHERE ID = ?");
    ...
    public void actualizarCritica(CriticaDTO critica, CriticaDTO oldCritica) {
        try {
            ...
            String sql = "UPDATE Criticas SET Titulo = ?, Puntuacion = ?, Texto = ?, Valoracion = ?, Owner = ?, CantidadValoraciones = ?, TituloEspectaculo = ?  WHERE ID = ?";
	...
	public CriticaDTO obtenerCriticaTitulo(String titulo) {
	    ...
	    try{
	        ...
	        String sqlString = "select c.Titulo, c.Puntuacion, c.Texto, c.Valoracion, c.Owner, c.CantidadValoraciones, c.TituloEspectaculo from `Criticas` c where c.Titulo = '" + titulo + "'";
	        ...
	        while (rs.next()) {
	            ...
	            critica.setTituloEspectaculo("c.TituloEspectaculo");
	        }
    ...
    public ArrayList<CriticaDTO> ObtenerTodasCriticas() {        
        ...
        try{
            ...
            String sqlString = "select c.Titulo, c.Puntuacion, c.Texto, c.Valoracion, c.Owner, c.CantidadValoraciones, c.ID, c.TituloEspectaculo from `Criticas` c";
            ResultSet rs = statement.executeQuery(sqlString);
            ...
            while (rs.next()) {
                ...
                critica.setOwner(rs.getString("c.Owner"));
                critica.setCantidadValoraciones(rs.getInt("c.CantidadValoraciones"));
                critica.setID(rs.getInt("c.ID"));
                critica.setTituloEspectaculo(rs.getString("c.TituloEspectaculo"));
