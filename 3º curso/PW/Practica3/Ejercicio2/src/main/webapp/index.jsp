...
<body>
	...
	%>
		...
		<p><a href="/Ejercicio2/mvc/views/registrar.jsp">Registrarse</a></p>
	<%
		} else {
	...
	<p>¡Bienvenido <%out.println(user.getNombre());%>!</p>
	<p>Hoy es <%out.println(dtf2.format(LocalDateTime.now())); %></p>
	<p>Lista de usuarios registrados en la base de datos: 
	<%
		ArrayList<UserDTO> listaUser = new ArrayList<UserDTO>();
		//ArrayList<UserDTO> lista = userManager.usuarios();
		listaUser = userManager.usuarios();
		for(int i=0; i<listaUser.size();i++){
			System.out.println(listaUser.get(i).getUsername());
		
	%></p>
	
	<p> Username: <%out.println(listaUser.get(i).getUsername()+" Tipo de Usuario: "+(listaUser.get(i).getTipoAsString()) +" Fecha de Registro: "+ (listaUser.get(i).getFecha()));%></p>
	<%
	}	
	%>
	<a href="/Ejercicio2/mvc/controllers/desconectarController.jsp">Desconectar</a>
